...
function NewTaskButton({ user, tasks, setTasks, users, setUsers, socket }) {
  ...
  useEffect(() => {
    if (
      taskInputs &&
      taskInputs.assignees.some((assignee) => assignee._id === user.id)
    ) {
      ...
      <Dialog>
        <DialogTrigger asChild>
          <Button className="gap-1">
            <CirclePlus className="h-5 w-5 ml-[-6px]" />
            New Task
          </Button>
        </DialogTrigger>
                      ...
                      {assignees.length > 0 ? (
                        <span className="flex">
                          {assignees.slice(0, 3).map((x, i) => {
                            return i === 2 && assignees.length > 3 ? (
                              <p key={i}>{x.name}&nbsp;...</p>
                            ) : i === 0 && assignees.length > 1 ? (
                              <p key={i}>{x.name},&nbsp;</p>
                            ) : i === 1 && assignees.length > 2 ? (
                              <p key={i}>{x.name},&nbsp;</p>
                            ) : (
                              <p key={i}>{x.name}</p>
                            );
